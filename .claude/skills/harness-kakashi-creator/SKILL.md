...
```
카카시 하네스에 오신 것을 환영합니다!

하네스는 프로젝트의 품질 관리와 워크플로우를 구조화하는 프레임워크입니다.
아직 초기화되지 않았으므로, 아래 순서대로 시작해 보세요.
...
하네스의 기본 구조를 생성합니다:
  ...
  ├── agents/tamer.md       ← 조련사 (기본 내장 에이전트)
  ├── knowledge/            ← 도메인 지식 저장소
  ├── engine/               ← 워크플로우 정의
  ├── docs/                 ← 버전 히스토리
  └── logs/                 ← 활동 로그

초기화 시 하네스 이름과 설명을 입력하면 완료됩니다.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📌 Step 2: 개선부 — 하네스 자체를 만들고 고치기
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

초기화 후, 하네스를 키워나가는 작업입니다:

  /harness-kakashi-creator 하네스를 설명해    ← 현재 상태 요약
  /harness-kakashi-creator 하네스를 개선해    ← 평가 후 개선안 도출
  /harness-kakashi-creator 새 에이전트 추가해 ← 새 역할 제안
  /harness-kakashi-creator 스킬 복사해        ← 기존 스킬을 하네스에 통합

개선부는 에이전트, 지식, 엔진 세 계층(3-Layer)으로 구성됩니다:
  Layer 1: knowledge/ — 도메인 지식, 방법론
  Layer 2: agents/    — 에이전트 역할 정의
  Layer 3: engine/    — 워크플로우, 오케스트레이션

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📌 Step 3: 수행부 — 하네스가 프로젝트를 위해 일하기
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

에이전트와 엔진이 정의되면, 실제 프로젝트 코드를 대상으로 실행합니다:

  /harness-kakashi-creator 전체 점검해   ← 전체 리뷰 엔진 실행
  /harness-kakashi-creator 변경 점검해   ← 변경사항만 대상 리뷰
  /harness-kakashi-creator 하네스 수행해 ← 전체 점검과 동일

모든 수행 결과는 자동으로 harness/logs/에 기록되고 평가됩니다.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

먼저 /harness-kakashi-creator init 으로 시작해 보세요!
```

### Case 2: 하네스가 있는 경우 (harness.config.json 존재)

harness.config.json에서 name, version, agents, engine 정보를 읽은 뒤 다음 튜토리얼을 출력한다:
...
하네스의 구조, 에이전트, 지식, 엔진을 추가하고 개선하는 작업입니다.

  /harness-kakashi-creator 하네스를 설명해     ← 현재 상태 요약 보고
  /harness-kakashi-creator 하네스를 개선해     ← 평가 후 개선안 도출
  /harness-kakashi-creator 하네스를 업데이트해 ← 구조/내용 갱신
  /harness-kakashi-creator 평가로그를 점검해   ← 기존 로그 분석
  /harness-kakashi-creator 새 에이전트 추가해  ← 새 역할 제안
  /harness-kakashi-creator 스킬 복사해         ← 기존 스킬을 하네스에 통합
  /harness-kakashi-creator init                ← 하네스 재초기화
...
📌 수행부 — 하네스가 프로젝트를 위해 일하기
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

정의된 에이전트와 엔진을 실제 프로젝트 코드에 대해 실행합니다.

  /harness-kakashi-creator 전체 점검해   ← 전체 리뷰 엔진 실행
  /harness-kakashi-creator 변경 점검해   ← 변경사항만 대상 리뷰
  /harness-kakashi-creator 하네스 수행해 ← 전체 점검과 동일

모든 수행 결과는 harness/logs/에 자동 기록되고 평가됩니다.
...
> **중요**: 튜토리얼 모드는 안내만 출력하고 끝난다. 로그를 기록하지 않는다.
